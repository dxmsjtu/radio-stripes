...
%UEs in the entire network. The channels are modeled as correlated Rayleigh fading and the MMSE estimator is used.
...
%INPUT:
%R                 = Matrix with dimension N x N x K x L where (:,:,k,l) is the spatial correlation matrix between AP l and UE k
...
%H            = Matrix with dimension L*N x K x numRealz with the true channel realizations. The matrix is organized in the
%               same way as Hhat.
%R_hat        = Matrix with dimension N x N x K x L where (:,:,k,l) is the spatial correlation matrix of the estimate between AP l and
